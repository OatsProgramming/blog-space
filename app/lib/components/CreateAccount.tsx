  ...
  return (
      <motion.div className='signInPage60' layout='preserve-aspect' layoutId='inputSwitch'>
        ...
        <form className="signInPageForm">
            <input className="formInput" type='email' placeholder="email" onChange={(e) => useAuth.setState((state) => ({createInfo: {...state.createInfo, email: e.target.value} }))}/>
            <label htmlFor='email' className='formLabelEmail'>Email</label>
            <input className='formInput' type='password' placeholder="password" onChange={(e) => useAuth.setState((state) => ({createInfo: {...state.createInfo, password: e.target.value}}))}/>
